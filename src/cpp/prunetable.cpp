...
prunetable::prunetable(const puzdef &pd, ull maxmem, int phase_id_)
  ...
  while ((size >> shardshift) > MEMSHARDS)
    shardshift++;
  if (quiet == 0)
    cout << log_prefix(phase_id) << "For memsize " << maxmem << " sh1 "
         << shift1 << " mul " << memmul << " sh2 " << shift2
         << " shardshift " << shardshift << endl;
  totpop = 0;
  ptotpop = 0;
  baseval = 0;
  wval = 0;
  if (quiet == 0)
    cout << log_prefix(phase_id) << "Trying to allocate "
  ...
  if (!readpt(pd)) {
    if (quiet == 0)
      cout << log_prefix(phase_id) << "Initializing memory in " << duration()
           << endl << flush;
    baseval = 1;
    filltable(pd, 0);
    filltable(pd, 1);
...
void prunetable::filltable(const puzdef &pd, int d) {
  ...
  ll ofillcnt = fillcnt;
  if (quiet == 0)
    cout << log_prefix(phase_id) << "Filling depth " << d << " val " << wval
         << endl;
  ...
  if (quiet == 0) {
    ...
    cout << log_prefix(phase_id) << "Filled depth " << d << " val " << wval
