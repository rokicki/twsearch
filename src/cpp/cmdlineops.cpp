...
ll maxwrong;
/*
 *   --multiphase moveset  (repeatable)
 *
 *   Each use appends one moveset to the phase pipeline.  N uses create an
 *   N-phase concurrent solve:
 *     Phase 0 uses moveset[0] and targets the subgroup of moveset[1]
 *     Phase 1 uses moveset[1] and targets the subgroup of moveset[2]
 *     ...
 *     Phase N-1 uses moveset[N-1] and targets the standard solved position.
 *
 *   If not given, the existing single-phase solve runs as before.
 */
static vector<string> multiphase_movesets;
static struct multiphaseopt : specialopt {
  multiphaseopt()
      : specialopt("--multiphase",
                   "moveset  Add a phase to the multiphase pipeline.  Repeat\n"
                   "  this option once per phase.  N uses → N-phase concurrent\n"
                   "  solve.  Phase k uses moveset k and targets the subgroup\n"
                   "  generated by moveset k+1 (last phase targets solved).\n"
                   "  Pair with --scramblealg or a scramble file.\n") {}
  virtual void parse_args(int *argc, const char ***argv) {
    if (*argc < 2)
      error("! --multiphase: missing moveset argument");
    (*argc)--;
    (*argv)++;
    multiphase_movesets.push_back(**argv);
  }
} registermultiphase;
void solvecmdline(puzdef &pd, const char *scr, generatingset *gs) {
  stacksetval p1(pd);
  pd.assignpos(p1, pd.solved);
  vector<allocsetval> mvs = parsemovelist_generously(pd, scr);
...
void multiphase_solveit(puzdef &pd, setval &p) {
  ...
  //   Phase k (1..N-1): movesets[k-1] moves, targets subgroup of movesets[k]
  //   Phase N: movesets[N-1] moves, targets the standard solved position.
  int nphases = n + 1;
  vector<phasespec> specs;
  specs.reserve(nphases);
